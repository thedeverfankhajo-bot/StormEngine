...
GlesRenderDevice::~GlesRenderDevice() {
    ...
    for (const auto& [handle, shader] : shaders_) {
        (void)handle;
...
bool GlesRenderDevice::updateBuffer(BufferHandle handle, const void* data, std::size_t size,
                                    std::size_t offset) {
    if (!handle.valid() || data == nullptr || size == 0 || buffers_.find(handle.id()) == buffers_.end())
        return false;
...
TextureHandle GlesRenderDevice::createTexture(const TextureDesc& desc) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, static_cast<GLsizei>(desc.width), static_cast<GLsizei>(desc.height),
...
bool GlesRenderDevice::submit(const DrawCommand& command) {
    if (!frameActive_ || !command.vertexBuffer.valid() || command.vertexCount == 0 || !command.vertexLayout.valid())
        return false;
    if (buffers_.find(command.vertexBuffer.id()) == buffers_.end() || command.baseVertex != 0)
        return false;
    if (!command.shader.valid())
        return false;

    const auto shaderIt = shaders_.find(command.shader.id());
    ...
    if (!pipelineReady_) {
        ...
        if (program_ == 0)
            return false;
        // A ShaderHandle refers to a compiled shader. The first real backend
        // pipeline is intentionally vertex-shader driven; a fragment shader
        // will be added as a separate pipeline resource in the next step.
        glAttachShader(static_cast<GLuint>(program_), static_cast<GLuint>(shaderIt->second));
