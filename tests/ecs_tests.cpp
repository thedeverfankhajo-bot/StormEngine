...
void testComponentStorageSwapBack() {
    ...
    storage.emplace(1, 10, 20);
    storage.emplace(2, 30, 40);
    storage.emplace(3, 50, 60);
...
void testQueryUsesMatchingEntities() {
    ...
    registry.emplace<Velocity>(a, 10, 0);

    registry.emplace<Position>(b, 2, 0);

    registry.emplace<Position>(c, 3, 0);
    registry.emplace<Velocity>(c, 30, 0);
    registry.emplace<Tag>(c, 99);
