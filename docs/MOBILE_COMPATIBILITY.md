...
The compatibility test matrix uses:
- Samsung Galaxy
- Xiaomi / Redmi / POCO
- Google Pixel
- OnePlus
...
A family entry is not certification for every model. Physical GPU/driver validation is required.
...
## Compatibility goal
The project is structured so the portable C++ core can build on Termux while the Android renderer is validated independently on representative device families.