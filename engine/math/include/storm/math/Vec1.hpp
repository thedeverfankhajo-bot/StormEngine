...
struct Vec1 {
    ...
    constexpr Vec1 operator+(Vec1 rhs) const noexcept { return {x + rhs.x}; }
    constexpr Vec1 operator-(Vec1 rhs) const noexcept { return {x - rhs.x}; }
    constexpr Vec1 operator-() const noexcept { return {-x}; }
    constexpr Vec1 operator*(float scalar) const noexcept { return {x * scalar}; }
    constexpr Vec1 operator/(float scalar) const noexcept { return {x / scalar}; }
    ...
    Vec1 normalized() const noexcept { return x == 0.0f || !std::isfinite(x) ? Vec1{} : Vec1{x > 0.0f ? 1.0f : -1.0f}; }
